...
public class MainActivity extends AppCompatActivity {
    ...
    private void handleCustomerPurchasesResponse(Message msg) {
        @SuppressWarnings("unchecked")
        java.util.HashMap<String, Integer> purchases = (java.util.HashMap<String, Integer>) msg.obj;
        productList.clear();
        if (purchases != null && !purchases.isEmpty()) {
            // Προσπάθησε να βρεις το Store και τα πραγματικά Product για να πάρεις τιμή/κατηγορία
            String storeName = etStoreName.getText().toString().trim();
            Store foundStore = null;
            // Αν έχεις ήδη λίστα Store, μπορείς να την χρησιμοποιήσεις. Εδώ απλά δημιουργούμε dummy Store.
            // Αν δεν έχεις, θα εμφανίζεται μόνο το όνομα προϊόντος και ποσότητα.
            for (String productName : purchases.keySet()) {
                String category = "";
                double price = 0.0;
                // Optional: Αν έχεις πρόσβαση σε Store/Products, βρες τιμή/κατηγορία
                // αλλιώς άφησε τα κενά/μηδέν
                productList.add(new Product(productName, category, purchases.get(productName), price));
            }
            adapter.notifyDataSetChanged();
            purchaseListView.setVisibility(View.VISIBLE);
            listViewHeader.setVisibility(View.VISIBLE);
